...
class ScanProcess(models.Model):
    """Background'da ishlayotgan tool process'larni boshqarish"""
    STATUS_CHOICES = [
        ('running', 'Ishlayapti'),
        ('completed', 'Tugallandi'),
        ('failed', 'Xatolik'),
    ...
    domain_name = models.CharField(max_length=255, verbose_name="Domain nomi")
    tool_type = models.CharField(max_length=50, verbose_name="Tool turi")
    process_id = models.IntegerField(null=True, blank=True, verbose_name="Process ID")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='running')
    start_time = models.DateTimeField(default=timezone.now, verbose_name="Boshlash vaqti")
    ...
    def __str__(self):
        return f"{self.domain_name} - {self.tool_type} ({self.status})"
    
    def is_running(self):
        return self.status == 'running'
    
    def add_output(self, output):
        """Output'ga yangi ma'lumot qo'shish"""
