    ...
    domainsList.innerHTML = domains.map((domain, index) => `
        <div class="domain-item fade-in" style="animation-delay: ${index * 0.1}s">
            ...
            <div class="domain-actions">
                <button class="btn btn-small btn-info" onclick="showScanProgress(${index})" id="progressBtn_${index}" style="display: none;">
                    🔍 Jarayon
                </button>
...
function startScan() {
    ...
    // Show progress buttons for each domain
    showDomainProgressButtons();
    ...
    .then(data => {
        if (data.success) {
            ...
            const message = data.message || 'Parallel tahlil muvaffaqiyatli bajarildi!';
...
function showDomainProgressButtons() {
    // Har bir domain uchun "Jarayon" tugmasini ko'rsatish
...
function showScanProgress(domainIndex) {
    const domain = domains[domainIndex];
    
    // Create compact progress modal for specific domain
    const modal = document.createElement('div');
    modal.id = `scanProgressModal_${domainIndex}`;
    modal.className = 'scan-progress-modal';
    modal.innerHTML = `
        <div class="scan-progress-modal-content">
            <div class="scan-progress-modal-header">
                <div class="scan-progress-modal-title">
                    ...
                    <h3>${domain} - Real-time Tahlil</h3>
                </div>
                <button class="scan-progress-modal-close" onclick="closeScanProgressModal(${domainIndex})">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </button>
            ...
            <div class="scan-progress-modal-body">
                <div class="scan-status-section">
                    <div class="scan-status-item">
                        <span class="status-label">Holat:</span>
                        <span class="status-value scanning">⏳ Tahlil qilinmoqda...</span>
                    </div>
                    <div class="scan-status-item">
                        <span class="status-label">Progress:</span>
                        <div class="progress-bar">
                            <div class="progress-fill" id="progressFill_${domainIndex}"></div>
                        </div>
                        <span class="progress-text" id="progressText_${domainIndex}">0%</span>
                    </div>
                </div>
                
                <div class="tools-progress-section">
                    <h4>🔧 Tool Tahlillari</h4>
                    <div class="tool-progress-list">
                        <div class="tool-progress-item" id="nmap_progress_${domainIndex}">
                            <div class="tool-icon">🌐</div>
                            <div class="tool-info">
                                <div class="tool-name">Nmap (Port & Vulnerability)</div>
                                <div class="tool-status pending">⏳ Kutilmoqda...</div>
                            </div>
                            <button class="btn btn-small btn-info" onclick="showToolLogs('${domainIndex}', 'nmap')" id="nmap_logs_btn_${domainIndex}">
                                📋 Loglar
                            </button>
                        </div>
                        <div class="tool-progress-item" id="sqlmap_progress_${domainIndex}">
                            <div class="tool-icon">💉</div>
                            <div class="tool-info">
                                <div class="tool-name">SQLMap (SQL Injection)</div>
                                <div class="tool-status pending">⏳ Kutilmoqda...</div>
                            </div>
                            <button class="btn btn-small btn-info" onclick="showToolLogs('${domainIndex}', 'sqlmap')" id="sqlmap_logs_btn_${domainIndex}">
                                📋 Loglar
                            </button>
                        </div>
                        <div class="tool-progress-item" id="gobuster_progress_${domainIndex}">
                            <div class="tool-icon">📁</div>
                            <div class="tool-info">
                                <div class="tool-name">Gobuster (Directory)</div>
                                <div class="tool-status pending">⏳ Kutilmoqda...</div>
                            </div>
                            <button class="btn btn-small btn-info" onclick="showToolLogs('${domainIndex}', 'gobuster')" id="gobuster_logs_btn_${domainIndex}">
                                📋 Loglar
                            </button>
                        </div>
                        <div class="tool-progress-item" id="xsstrike_progress_${domainIndex}">
                            <div class="tool-icon">🕷️</div>
                            <div class="tool-info">
                                <div class="tool-name">XSStrike (XSS)</div>
                                <div class="tool-status pending">⏳ Kutilmoqda...</div>
                            </div>
                            <button class="btn btn-small btn-info" onclick="showToolLogs('${domainIndex}', 'xsstrike')" id="xsstrike_logs_btn_${domainIndex}">
                                📋 Loglar
                            </button>
                        </div>
                    </div>
                ...
                <div class="real-time-output-section">
                    <h4>📊 Real-time Natijalar</h4>
                    <div class="output-container">
                        <div class="output-content" id="outputContent_${domainIndex}">
                            <div class="output-line">🚀 Tahlil boshlanmoqda...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    startRealTimeProgress(domainIndex);
}

function closeScanProgressModal(domainIndex) {
...
function startRealTimeProgress(domainIndex) {
    const domain = domains[domainIndex];
    ...
    const progressInterval = setInterval(() => {
        ...
        // Update progress bar
        const progressFill = document.getElementById(`progressFill_${domainIndex}`);
        const progressText = document.getElementById(`progressText_${domainIndex}`);
        ...
        updateToolStatus(domainIndex, progress);
        
        // Add output lines
        addOutputLine(domainIndex, progress);
        
        if (progress >= 100) {
            clearInterval(progressInterval);
            completeScan(domainIndex);
        }
    }, 1000);
}

function updateToolStatus(domainIndex, progress) {
    const tools = [
        { id: 'nmap', name: 'Nmap', threshold: 25 },
        { id: 'sqlmap', name: 'SQLMap', threshold: 50 },
        { id: 'gobuster', name: 'Gobuster', threshold: 75 },
        { id: 'xsstrike', name: 'XSStrike', threshold: 90 }
    ];
    
    tools.forEach(tool => {
        const toolElement = document.getElementById(`${tool.id}_progress_${domainIndex}`);
        if (toolElement && progress >= tool.threshold) {
...
function addOutputLine(domainIndex, progress) {
    const outputContent = document.getElementById(`outputContent_${domainIndex}`);
    if (!outputContent) return;
    
    const messages = [
        '🔍 Domain tahlil qilinmoqda...',
        '🌐 Nmap port skanerlash boshlanmoqda...',
        '💉 SQLMap SQL injection testlash...',
        '📁 Gobuster directory discovery...',
        '🕷️ XSStrike XSS detection...',
        '✅ Barcha tool\'lar tugallandi!',
    ...
    if (messageIndex < messages.length) {
        ...
        outputLine.textContent = messages[messageIndex];
        outputContent.appendChild(outputLine);
        outputContent.scrollTop = outputContent.scrollHeight;
    }
}

function completeScan(domainIndex) {
    const outputContent = document.getElementById(`outputContent_${domainIndex}`);
    if (outputContent) {
        const completionLine = document.createElement('div');
        completionLine.className = 'output-line completion';
        completionLine.textContent = '🎉 Tahlil muvaffaqiyatli tugallandi!';
        outputContent.appendChild(completionLine);
        outputContent.scrollTop = outputContent.scrollHeight;
    }
}

function showToolLogs(domainIndex, toolType) {
    const domain = domains[domainIndex];
    const toolNames = {
        'nmap': 'Nmap (Port & Vulnerability)',
        'sqlmap': 'SQLMap (SQL Injection)',
        'gobuster': 'Gobuster (Directory)',
        'xsstrike': 'XSStrike (XSS)'
    };
    ...
    // Create tool logs modal
    const modal = document.createElement('div');
    modal.id = `toolLogsModal_${domainIndex}_${toolType}`;
    modal.className = 'tool-logs-modal';
    modal.innerHTML = `
        <div class="tool-logs-modal-content">
            <div class="tool-logs-modal-header">
                <div class="tool-logs-modal-title">
                    <span class="tool-icon-large">${toolIcons[toolType]}</span>
                    <div class="tool-title-text">
                        <h3>${toolNames[toolType]}</h3>
                        <p>${domain}</p>
                    </div>
                </div>
                <button class="tool-logs-modal-close" onclick="closeToolLogsModal('${domainIndex}', '${toolType}')">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </button>
            ...
            <div class="tool-logs-modal-body">
                <div class="tool-status-section">
                    <div class="tool-status-item">
                        <span class="status-label">Tool Holati:</span>
                        <span class="status-value" id="tool_status_${domainIndex}_${toolType}">⏳ Ishlayapti...</span>
                    </div>
                    <div class="tool-status-item">
                        <span class="status-label">Boshlangan:</span>
                        <span class="status-value">${new Date().toLocaleTimeString()}</span>
                    </div>
                </div>
                
                <div class="tool-command-section">
                    <h4>🔧 Bajarilayotgan Buyruq</h4>
                    <div class="command-display" id="command_display_${domainIndex}_${toolType}">
                        <code>${getToolCommand(toolType, domain)}</code>
                    </div>
                </div>
                ...
                <div class="tool-output-section">
                    ...
                    <div class="tool-output-container">
                        <div class="tool-output-content" id="tool_output_${domainIndex}_${toolType}">
                            <div class="output-line">🚀 ${toolNames[toolType]} ishga tushirilmoqda...</div>
                        </div>
                    </div>
                </div>
                
                <div class="tool-actions">
                    <button class="btn btn-warning" onclick="stopTool('${domainIndex}', '${toolType}')" id="stop_btn_${domainIndex}_${toolType}">
                        ⏹️ To'xtatish
                    </button>
                    <button class="btn btn-secondary" onclick="clearToolOutput('${domainIndex}', '${toolType}')">
    ...
    // Start real-time tool output simulation
    startToolOutputSimulation(domainIndex, toolType);
}

function closeToolLogsModal(domainIndex, toolType) {
    const modal = document.getElementById(`toolLogsModal_${domainIndex}_${toolType}`);
    if (modal) {
        modal.style.opacity = '0';
        modal.style.transform = 'scale(0.9)';
        setTimeout(() => {
...
function getToolCommand(toolType, domain) {
    const commands = {
        'nmap': `nmap -sS -sV -O --script=vuln -T4 ${domain}`,
        'sqlmap': `sqlmap -u https://${domain} --batch --random-agent --level=1 --risk=1`,
        'gobuster': `gobuster dir -u https://${domain} -w wordlist.txt -t 10`,
        'xsstrike': `xsstrike --url https://${domain} --blind --skip-dom --threads 5`
    };
    return commands[toolType] || 'Command not found';
}

function startToolOutputSimulation(domainIndex, toolType) {
    const toolOutput = document.getElementById(`tool_output_${domainIndex}_${toolType}`);
    const toolStatus = document.getElementById(`tool_status_${domainIndex}_${toolType}`);
    const stopBtn = document.getElementById(`stop_btn_${domainIndex}_${toolType}`);
    
    if (!toolOutput || !toolStatus) return;
    
    // Tool-specific output messages
    const toolMessages = {
        'nmap': [
            '🔍 Port skanerlash boshlanmoqda...',
            '📡 TCP SYN scan ishlayapti...',
            '🔍 Port 80 ochiq (HTTP)',
            '🔍 Port 443 ochiq (HTTPS)',
            '🔍 Port 22 ochiq (SSH)',
            '🌐 Service detection ishlayapti...',
            '🔍 Apache 2.4.41 topildi',
            '🔍 OpenSSH 8.2p1 topildi',
            '⚠️ Vulnerability scripts ishlayapti...',
            '🔍 CVE-2021-41773 potentsial zaiflik topildi',
            '✅ Port skanerlash tugallandi'
        ],
        'sqlmap': [
            '💉 SQL injection testlash boshlanmoqda...',
            '🔍 GET parametrlari tahlil qilinmoqda...',
            '🔍 POST data tahlil qilinmoqda...',
            '🔍 Cookie parametrlari tekshirilmoqda...',
            '💉 Boolean-based injection testlash...',
            '💉 Time-based injection testlash...',
            '💉 Error-based injection testlash...',
            '🔍 Database type aniqlanmoqda...',
            '🔍 MySQL 8.0 topildi',
            '⚠️ SQL injection zaiflik topilmadi',
            '✅ SQL injection testlash tugallandi'
        ],
        'gobuster': [
            '📁 Directory discovery boshlanmoqda...',
            '🔍 Common directories tekshirilmoqda...',
            '📁 /admin topildi (200)',
            '📁 /login topildi (200)',
            '📁 /wp-admin topildi (403)',
            '📁 /backup topildi (200)',
            '📁 /config topildi (403)',
            '📁 /api topildi (200)',
            '📁 /images topildi (200)',
            '📁 /css topildi (200)',
            '✅ Directory discovery tugallandi'
        ],
        'xsstrike': [
            '🕷️ XSS detection boshlanmoqda...',
            '🔍 URL parametrlari tahlil qilinmoqda...',
            '🔍 Form fields tahlil qilinmoqda...',
            '💉 Reflected XSS testlash...',
            '💉 Stored XSS testlash...',
            '💉 DOM XSS testlash...',
            '🔍 Payload generation...',
            '💉 <script>alert(1)</script> testlash...',
            '💉 <img src=x onerror=alert(1)> testlash...',
            '⚠️ XSS zaiflik topilmadi',
            '✅ XSS detection tugallandi'
        ]
    };
    
    const messages = toolMessages[toolType] || [];
    let messageIndex = 0;
    let isRunning = true;
    
    // Store interval ID for stopping
    window[`tool_interval_${domainIndex}_${toolType}`] = setInterval(() => {
        if (!isRunning || messageIndex >= messages.length) {
            clearInterval(window[`tool_interval_${domainIndex}_${toolType}`]);
            if (messageIndex >= messages.length) {
                toolStatus.textContent = '✅ Tugallandi';
                toolStatus.className = 'status-value completed';
        ...
        outputLine.className = 'output-line';
        outputLine.textContent = messages[messageIndex];
        toolOutput.appendChild(outputLine);
    ...
    if (stopBtn) {
        stopBtn.onclick = () => {
            isRunning = false;
            clearInterval(window[`tool_interval_${domainIndex}_${toolType}`]);
            toolStatus.textContent = '⏹️ To\'xtatildi';
            toolStatus.className = 'status-value stopped';
            stopBtn.style.display = 'none';
        };
    }
}

function stopTool(domainIndex, toolType) {
    // This function is handled in startToolOutputSimulation
}

function clearToolOutput(domainIndex, toolType) {
    const toolOutput = document.getElementById(`tool_output_${domainIndex}_${toolType}`);
    if (toolOutput) {
        toolOutput.innerHTML = '<div class="output-line">🗑️ Natijalar tozalandi</div>';
    }
}
